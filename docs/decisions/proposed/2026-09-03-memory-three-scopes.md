...
> 状态:proposed · 提出 2026-09-03 · 拍板 2026-09-03(口头,实现后移入 implemented) · 来源 [会话与 agent 集群](../../design/sessions.md) §2 · 替代 2026-09-01「记忆用户级、跨项目共享」
...
`remember` 的三种 `scope` 各落到对应目录;两段不同 workspace 的 session 看不到彼此的 project 级记忆、看得到同一份 user 级;dream 跑完只有 session 级目录有变化。
