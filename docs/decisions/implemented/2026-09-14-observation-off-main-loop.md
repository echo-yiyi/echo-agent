...
- **产品定了过期规则**（2026-09-15）：`echo-agent` / `echo-coding` 各自声明「只留最近 30 天」（`Product.observationExpiry`），`mainFor()` 在启动时不等地扫一遍会话根下每一段。按会话根扫是因为缺省每次启动都新建一段——只清当前这一段等于什么都不清。core 仍然一条规则都不给。
...
- ~~`ObservationPersistenceStatus` 的 `lost-lease` 与缺口原因 `lease_lost` 不再有入口，类型还在公开面上~~ **2026-09-15 已删**：连同同样没有写者的 `degraded` / `recovering`、`store_failure` / `canonical_flush_timeout`、run 终态 `interrupted`、envelope 的 `sourceSeq` 一起（用户拍板，见设计文档 §8）。`RunObservationHeader.persistence` 整个字段 2026-09-16 也删了——它唯一的写者写死 `stored`，恒等于一个值的字段没有信息。
