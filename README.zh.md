...
一个自主 agent 的运行时与官方 CLI：既可以使用装配好的完整 runtime，也可以直接基于底层 engine 构建；同一个 agent 既能交互运行，也能接入 Unix 管道。
...
| `echo-agent` | 官方 CLI，支持交互与管道两种形态 |
| `@echo-agent/coding-agent` | 带 workspace 和 shell extensions 的 coding preset |
