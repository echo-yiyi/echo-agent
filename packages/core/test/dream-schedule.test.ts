...
import { createAgentMemories, memoryCreate, shouldDream } from "../src/memory/harness.ts";
...
test("门满足 → 回到 idle 自动整理并提交（装配方不写 timer 也不派 subagent）", async () => {
  ...
  expect(await shouldDream(mem)).toBe(true);
  ...
  const state = await readDreamState(mem.dir);
  expect(state.lastAt).not.toBeNull(); // 提交了
  expect(state.startedAt).toBeNull(); // 锁放了
  expect(await shouldDream(mem)).toBe(false); // 计数清零，短期内不会再跑
});
...
test("门不满足 → 不跑（写入数没到）", async () => {
  ...
  expect((await readDreamState(mem.dir)).lastAt).toBeNull(); // 没跑过
});
...
test("autoDream 关着 → 不跑（缺省就是关的，`start()` 才打开）", async () => {
  ...
  await settle();
  expect((await readDreamState(mem.dir)).lastAt).toBeNull();
});
...
test("前台抢占：整理真的被打断、不提交，且前台等它停下来才开工", async () => {
  // 旧版没有控制时序，断言是「锁没挂超过 60 秒」——那证明不了抢占发生过。
  // 这版让 dream 的流**可控地挂住**，抢占点就落在确定的位置上。
  const mem = await readyMemory();
  ...
  // 整理进行中：锁已经上了
  expect((await readDreamState(mem.dir)).startedAt).not.toBeNull();

  let promptDone = false;
  const second = agent.prompt("第二次立刻来").then(() => {
    promptDone = true;
  });

  // **前台必须等 dream 真的停**：dream 还挂着的时候，第二次 prompt 不该完成
  await new Promise((r) => setTimeout(r, 20));
  expect(promptDone, "前台没等 dream 收完就开工了").toBe(false);

  releaseDream();
  await second;
  await settle();

  // dream 被中断 → **不提交**：lastAt 仍是 null
  expect((await readDreamState(mem.dir)).lastAt).toBeNull();
  // 前台两轮都跑完了——抢占没有把前台也带停
...
test("进程重启：dream 状态在盘上，新实例接着算（不是每次起来都重新攒）", async () => {
  ...
  await a.prompt("干活");
  await settle();
  expect((await readDreamState(dir)).lastAt).not.toBeNull();

  // 换个进程：同一个盘，新的 memories 实例（内存计数从零开始）
  const second = createAgentMemories(dir, { dream: { minWritesSinceLast: 2, minFiles: 2 } });
  // 刚整理过 → 距上次时间太近，门不该放行。**这条正是「状态在盘上」的判据**：
  // 如果 dream 状态只活在内存里，新实例会以为从没整理过、立刻又跑一次。
  expect(await shouldDream(second)).toBe(false);
});
...
test("并发写记忆不丢计数——「单写者」防的是跨进程，防不了进程内并发", async () => {
  ...
  // 两次写都得算上
  expect((await readDreamState(inner)).writes).toBe(2);
  ...
  const mem = createAgentMemories(
    {
      ...
      write: (p, c) => slow.write(p, c),
      remove: (p) => slow.remove(p),
      list: (p) => slow.list(p),
    },
    { dream: { minWritesSinceLast: 1, minFiles: 1 } },
  );
...
test("门控读盘失败 → 报诊断，不变成 unhandled rejection", async () => {
  ...
  const dir = new InMemoryDir();
  const mem = createAgentMemories(dir, { dream: { minWritesSinceLast: 0, minFiles: 0 } });
  mem.dir = {
    read: async () => {
      throw new Error("盘挂了");
    },
...
test("轮次门真的接上了：turn_end 会被计数（此前 observer 压根没订阅）", async () => {
  const dir = new InMemoryDir();
  const mem = createAgentMemories(dir, { dream: { minTurnsSinceLast: 1 } });
  const agent = new Agent({
    model: FAKE_MODEL,
    streamFunction: scriptedStreamFn([textTurn("答完")]),
    memory: mem,
  ...
  expect((await readDreamState(dir)).turns).toBe(0);
  await agent.prompt("一轮");
  expect((await readDreamState(dir)).turns).toBeGreaterThan(0);
});
