...
//   - 一次只有一个 permit，按到达顺序发。来源只有 user 与 inbox，都是 foreground；记忆的提取与整理是隔离子循环，
//     走各自的通道（`memory/channel.ts`），不经这里——maintenance 那一档随 dream 一起退场（2026-09-14）；
//   - 每个 request 的 execute **至多调用一次**；rejected 的零次；
//   - 每个 ticket 的 settled **恰好 fulfill 一次**，绝不 reject：三种正常 rejected 也是 fulfill；
...
export type AdmissionDeps = Readonly<{
  ...
  binding(input: { source: RunSource; purpose: "foreground" }): RunModelBinding;
  /**
...
export class StandaloneRunAdmission implements AgentAdmissionPort {
  ...
  get activeRun(): Readonly<{ runId: string; source: RunSource; purpose: "foreground" }> | null {
    const a = this.active;
  ...
  enqueue<TResult extends LoopResult>(
    ...
    this.deps.assertReserved?.(request.reservationId, request.reservedRecordIds);
    return this.submit({ kind: "inbox" }, execute);
  }
  ...
  admitUser<TResult extends LoopResult>(
    ...
    return this.submit({ kind: "user" }, execute);
  }
  ...
  async close(reason: ClosedReason): Promise<void> {
    this.closed = reason;
    for (const s of this.queue.splice(0)) s.settle({ kind: "rejected", reason });
    const a = this.active;
    if (a !== null) {
      a.controller.abort();
      await a.done;
  ...
  private submit<TResult extends LoopResult>(
    ...
    const slot: Slot = { requestId, source, purpose: "foreground", execute, settle };
    if (this.closed !== null) {
  ...
  private pump(): void {
    if (this.active !== null) return; // 它 close 后 run() 末尾会再 pump
  ...
  private async run(slot: Slot): Promise<void> {
    ...
    let runId = "run:unassigned";
    let result: AgentAdmissionResult | null = null;
    try {
      runId = this.deps.runId?.(slot.source) ?? `run:${crypto.randomUUID()}`;
