...
//
// 两种实现共用这里的错误与进程内互斥：`FileDir.lock` 走带递增编号的锁（`generation-lock.ts`，与 lease
// 同一个实现），跨实例、跨进程互斥，持有者崩溃后自动接管；`InMemoryDir.lock` 与**没有 lock 原语的字节面**
// （按对象）只在进程内互斥，用下面的 `NameMutex`——进程内的持有者不会「崩了却还占着」。
//
// 活着的持有者占着、等到超时，就抛 `StorageLockBusy`，让调用方明确报冲突，报错里带着持有者与锁的位置。
