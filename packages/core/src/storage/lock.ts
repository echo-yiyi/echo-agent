...
export type Lease = {
   ...
   *
   * **这不是抢占**：锁不会被从活着的持有者手里夺走，是持有者自己让出来的——
   * 变的只是持有者可以声明「人来了我就走」。
   */
...
export interface StateLock {
   ...
   *
   * **core 不抢占**：返回 `null` 就是拿不到，core 自己不猜「对面是不是死了」。
   * 持有者死了之后能不能接管由实现决定并写进它的判据：文件锁在能**确认**持有者已死时接管
   * （同一台机器、pid 查无此号，`storage/generation-lock.ts`），带 TTL 的实现在租约过期后接管。
   */
  acquire(opts: {
    holder: string;
    /**
   ...
   * 拿不到锁 = 持有者还活着、确认不了死活（别的机器、pid 被复用）或锁坏了——后两种要人来清，
   * 而人去清的前提是**看得见是谁占着**。
   */
