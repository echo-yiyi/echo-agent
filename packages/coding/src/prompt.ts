...
All file tools work inside the workspace: relative paths resolve from its root, and paths outside it are refused. Find files with glob and search contents with grep rather than running find or rg through bash. Read a file with read_file before editing it. Change an existing file with edit_file (exact, unique match; replace_all for a rename); use write_file only to create a file or rewrite it entirely.`;

export const SHELL_TOOLS = `# Shell
Each bash call runs in a fresh shell in the workspace root: cd and variables do not carry over, so put \`cd dir && …\` inside the command. Commands time out after 120 s by default; for long-running processes (servers, watchers) pass background: true and keep working — you are notified when the job ends. A non-zero exit code comes back as an error with the output; read it before deciding the command worked. Today's date is not in your context; run \`date\` when it matters.`;
