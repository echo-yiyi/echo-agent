      ...
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            'import { createEcho, createProvider, createProviderStreams, toolOk } from "@echo-agent/core";',
            'import { scriptedDialect, textTurn, toolTurn } from "@echo-agent/core/testing";',
            "",
            'process.env.ECHO_HOME = mkdtempSync(join(tmpdir(), "echo-dist-home-"));',
            "const echo = await createEcho({",
            '  provider: createProvider({ id: "s", auth: { apiKey: { resolve: async () => ({ apiKey: "x" }) } }, models: [{ id: "only", api: "fake" }], api: createProviderStreams(scriptedDialect([toolTurn("c1", "ping", {}), textTurn("done")])) }),',
            '  stateDir: mkdtempSync(join(tmpdir(), "echo-dist-state-")),',
            "  allowNetwork: false,",
            "  withoutMemory: true,",
            "  extensionDirs: [],",
            "  agent: {",
            ...
            "const messages = echo.agent.state.messages;",
            "await echo.stop();",
            "console.log(JSON.stringify({",
            "  outcome: result.outcome.kind,",
            "  messages: messages.length,",
            '  sawTool: messages.some((m) => m.role === "toolResult"),',
            "}));",
          ].join("\n"),
        );
  ...
  test(
    ...
    () => {
      ...
      try {
        ...
        // 类型不只是「文件在」，还得真带出符号来。
        expect(readFileSync(join(installed, "dist", "index.d.ts"), "utf8")).toContain("createEcho");
      } finally {
        cleanup();
      }
    },
